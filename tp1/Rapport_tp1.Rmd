...
Estimateur_Esperance_limite <- sapply(1:length(limite_u), function(u)
    mean(sapply(1:100, function(i) min(data[i], limite_u[u]))))   
```


```{R echo=FALSE}
resultats <- data.frame("Percentile" = k, "Limite u" = limite_u, "Espérance limitée" = Estimateur_Esperance_limite)
knitr::kable(resultats, align = c('c','c','c'), col.names = c('Percentile', 'Limite u', 'Espérance limitée'), caption = "Estimation de l'espérance limitée")
```
...
\begin{align*}
    ...
    &=[\hat{S}_n(t)]^2\sum_{i=1}^t\frac{S_i}{r_i(S_i-r_i)}
\end{align*}
